...
export default async function forgotPassword(prevState, formData) {
	...
	if (!user.password) {
		try {
			const { data, error } = await resend.emails.send({
				...
				react: (
					<>
						...
						<p>
							You don't have a password stored in our database.
							This usually means you logged in with Google. If you
							are unable to login with Google, contact
							kieranc0808@gmail.com
						</p>
						<br />
						<small>
							If you did not request this email, you can ignore
							it. This is not part of a subscriber list.
						</small>
					</>
				)
			});
			...
			return { payload: formData, success: true };
		} catch (error) {
			errors.push(error);
		}
