...
export async function POST(req) {
    try {
        ...
        return NextResponse.json({ error: "Book categories updated successfully" }, { status: 200 });
    } catch (error) {
        console.error("Error updating book categories:", error);
        return NextResponse.json({ error: "Failed to update book categories" }, { status: 500 });
    }
}